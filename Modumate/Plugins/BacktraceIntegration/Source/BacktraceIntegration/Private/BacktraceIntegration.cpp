...
// We never want to go to the installed engine config
#if 0
	...
	if (LoadProjectFile(projContents)) {
		...
		if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
		{
			...
			for (auto el : arr)
			{
				...
				FString Name;
				//@third party BEGIN MODUMATE - fix warning C4456: declaration of 'success' hides previous local declaration
...
void FBacktraceIntegration::SetupPostBuildStep()
{
	...
	//@third party BEGIN MODUMATE - disable uploads in Development configuration
	auto str_to_add = FString::Printf(TEXT("%s %s/%s %s %s %s %s %s %s %s %s %s %s %s"),
		TEXT("if NOT \"$(TargetConfiguration)\" == \"Development\""),
	//@third party END MODUMATE - disable uploads in Development configuration
