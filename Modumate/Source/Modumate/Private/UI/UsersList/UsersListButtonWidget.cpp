...
void UUsersListButtonWidget::SetupFromPlayerState(const class AEditModelPlayerState* InOwnerPlayerState)
{
	if (InOwnerPlayerState && !InOwnerPlayerState->IsActorBeingDestroyed())
	{
		...
		FLinearColor color = InOwnerPlayerState->GetClientColor();
		Setup(firstName, lastName, color);
	}
}

void UUsersListButtonWidget::Setup(const FString& FirstName, const FString& LastName, const FLinearColor& Color)
{
	// Text string
	FString firstName = FirstName;
	FString lastName = LastName;
	if (firstName.Len() > 0)
	{
	...
	// Text color
	FLinearColor textColor = UModumateUIStatics::GetTextColorFromBackgroundColor(Color);
	TextBlock_UserName->SetColorAndOpacity(FSlateColor(textColor));

	// Background color
	ModumateButton->SetBackgroundColor(Color);
}
