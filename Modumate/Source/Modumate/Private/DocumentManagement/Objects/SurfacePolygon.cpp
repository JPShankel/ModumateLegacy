...
namespace Modumate
{
	...
	void FMOISurfacePolygonImpl::SetFromDataRecordAndRotation(const FMOIDataRecordV1 &dataRec, const FVector &origin, const FQuat &rotation)
	{
	...
	void FMOISurfacePolygonImpl::SetFromDataRecordAndDisplacement(const FMOIDataRecordV1 &dataRec, const FVector &displacement)
	{
		if (ensure(dataRec.ControlPoints.Num() == MOI->GetControlPoints().Num()))
		{
			for (int32 i = 0; i < dataRec.ControlPoints.Num(); ++i)
	...
	void FMOISurfacePolygonImpl::UpdateDynamicGeometry()
	{
		if (!GotGeometry)
		{
			return;
	...
	void FMOISurfacePolygonImpl::OnSelected(bool bNewSelected)
	{
	...
	void FMOISurfacePolygonImpl::UpdateConnectedVisuals()
	{
		if (MOI)
		{
