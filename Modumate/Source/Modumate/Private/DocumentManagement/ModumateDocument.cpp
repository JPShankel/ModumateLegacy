...
void FModumateDocument::SetAssemblyForObjects(UWorld *world,TArray<int32> ids, const FModumateObjectAssembly &assembly)
{
	...
	ur->Redo = [this, ur, ids, assembly]()
	{
		...
		for (auto id : ids)
		{
			...
			if (ob != nullptr)
			{
				originals.Add(ob, ob->ObjectAssembly);
				ob->ObjectAssembly = assembly;
...
void FModumateDocument::UpdateControlValues(int32 id, const TArray<FVector> &controlPoints, const TArray<int32> &controlIndices)
{
	...
	ur->Redo = [ur, id, moi, controlPoints, controlIndices, this]()
	{
		...
		ur->Undo = [id, oldCPs, oldCIs, this]()
		{
			...
			if (moi != nullptr)
			{
				moi->ControlPoints = oldCPs;
				moi->ControlIndices = oldCIs;
				moi->UpdateGeometry();
			}
		};
	};
...
void FModumateDocument::AdjustMoiHoleVerts(int32 id, const FVector &location, const TArray<FVector>& holeVerts)
{
	...
	TArray<FVector> oldHoleVerts = holeObject->ControlPoints;
	...
	ur->Redo = [ur, id, holeVerts, oldHoleVerts, oldLocation, newLocation, this]()
	{
		...
		holeRedoObj->ControlPoints = holeVerts;
		...
		ur->Undo = [this, id, oldLocation, oldHoleVerts]()
		{
			...
			{
				holeUndoObj->ControlPoints = oldHoleVerts;
...
bool FModumateDocument::RestoreObjectImpl(FModumateObjectInstance *obj)
{
	if (obj && (DeletedObjects.FindRef(obj->ID) == obj))
	{
		...
		obj->RestoreActor();
		obj->SetParentObject(GetObjectById(obj->Parent), true);
...
bool FModumateDocument::RestoreChildrenImpl(FModumateObjectInstance *obj)
{
	if (obj)
	{
		for (int32 childID : obj->Children)
		{
...
bool FModumateDocument::UpdateGraphObjects(UWorld *World)
{
	...
	if (VolumeGraph.CleanGraph(cleanedVertices, cleanedEdges, cleanedFaces))
	{
		...
		for (int32 edgeID : cleanedEdges)
		{
			...
			edgeObj->ControlPoints = { startVertex->Position, endVertex->Position };
		...
		for (int32 faceID : cleanedFaces)
		{
			...
			faceObj->ControlPoints = graphFace->CachedPositions;
...
void FModumateDocument::DeleteObjects(const TArray<FModumateObjectInstance*> &obs, bool bAllowRoomAnalysis, bool bDeleteConnected)
{
	...
	ur->Redo = [this, ur, allobs, world, graphDelta, bApplyDelta, deletedObjParentPlaneIDs]()
	{
		...
		for (auto *obj : allobs)
		{
			...
			if (obj->ObjectAssembly.ObjectType == EObjectType::OTDoor)
			{
				doorsDirty = true;
			}
			if (obj->ObjectAssembly.ObjectType == EObjectType::OTWindow)
			{
				windowsDirty = true;
			}
...
void FModumateDocument::UpdateLineSegment(int32 id, const FVector &p1, const FVector &p2)
{
	...
	{
		FVector oldP1 = ob->ControlPoints[0];
		FVector oldP2 = ob->ControlPoints[1];

		ob->ControlPoints[0] = p1;
		ob->ControlPoints[1] = p2;
		ob->UpdateGeometry();

		ur->Undo = [oldP1, oldP2, ob]()
		{
			ob->ControlPoints[0] = oldP1;
			ob->ControlPoints[1] = oldP2;
			ob->UpdateGeometry();
		};
	};
...
int32 FModumateDocument::MakeMetaPlaneHostedObject(UWorld *World, int32 MetaPlaneID, EObjectType ObjectType, const FModumateObjectAssembly &Assembly, float PlaneOffsetPCT, bool bInverted)
{
	...
	ur->Redo = [this, ur, id, World, MetaPlaneID, ObjectType, Assembly, PlaneOffsetPCT, bInverted]()
	{
		...
		if (!ensureAlways(planeObj->Children.Num() <= 1))
		{
			return;
		}
		...
		if (planeObj->Children.Num() == 1)
		{
			previousObj = GetObjectById(planeObj->Children[0]);
		...
		if (previousObj)
		{
			...
			TArray<int32> prevChildIDs = previousObj->Children;
		...
		ur->Undo = [this, World, MetaPlaneID, hostedObj, previousObj]()
		{
			...
			if (previousObj)
			{
				...
				TArray<int32> hostChildIDs = hostedObj->Children;
...
bool FModumateDocument::FinalizeGraphDelta(FGraph3D &TempGraph, FGraph3DDelta &Delta)
{
	...
	for (auto &kvp : parentIDToChildrenIDs)
	{
		...
		if (parentObj && parentObj->Children.Num() > 0)
		{
			for (int32 childIdx = 0; childIdx < parentObj->Children.Num(); childIdx++)
			{
				auto childObj = GetObjectById(parentObj->Children[childIdx]);
				...
				for (int32 childFaceID : kvp.Value)
				{
					if (!idsWithObjects.Contains(childFaceID))
					{
						...
						Delta.ParentIDUpdates.Add(newObjID, FGraph3DHostedObjectDelta(parentObj->Children[childIdx], MOD_ID_NONE, childFaceID));
						childFaceIDToHostedID.Add(childFaceID, newObjID);
						idsWithObjects.Add(childFaceID);
					}
					else
					{
						rejectedObjects.Add(parentObj->Children[childIdx]);
					}
				}
	...
	for (int32 objID : deletedObjIDs)
	{
		...
		if (obj && obj->Children.Num() > 0)
		{
			for (int32 childIdx = 0; childIdx < obj->Children.Num(); childIdx++)
			{
				Delta.ParentIDUpdates.Add(obj->Children[childIdx], FGraph3DHostedObjectDelta(MOD_ID_NONE, objID, MOD_ID_NONE));
			}
		}
	}
...
int32 FModumateDocument::MakeFinish(
	...
	int32 faceIndex,
	const FModumateObjectAssembly &newFinishAssembly)
{
	UE_LOG(LogCallTrace, Display, TEXT("ModumateDocument::MakeFinish"));

	// Determine which side of the wall the finish should go on, and if it already has one on that side.
	FModumateObjectInstance *parentObj = GetObjectById(targetObjectID);
	if (!parentObj)
	{
		UE_LOG(LogCallTrace, Warning, TEXT("ModumateDocument::MakeFinish - invalid parent ID %d!"), targetObjectID);
		return 0;
	}
	...
	ur->Redo = [this, world, ur, id, faceIndex, targetObjectID, newFinishAssembly]()
	{
		UE_LOG(LogCallTrace, Display, TEXT("ModumateDocument::MakeFinish::Redo"));

		FModumateObjectInstance *finishObj = nullptr;
		FModumateObjectInstance *parentObj = GetObjectById(targetObjectID);

		bool finishAlreadyExisted = false;
		FModumateObjectAssembly oldFinishAssembly = FModumateObjectAssembly();

		if (parentObj)
		{
			auto siblings = parentObj->GetChildObjects();
			for (auto *sibling : siblings)
			{
				int32 siblingFaceIndex = UModumateObjectStatics::GetFaceIndexFromFinishObj(sibling);
				if (siblingFaceIndex == faceIndex)
				{
					finishObj = sibling;
					break;
				}
			...
			finishAlreadyExisted = (finishObj != nullptr);

			if (finishAlreadyExisted)
			{
				oldFinishAssembly = finishObj->ObjectAssembly;
				finishObj->ObjectAssembly = newFinishAssembly;
				// TODO: add SetupGeometry() here once SetObjectLocation stops calling it unnecessarily
			}
			else
			{
		...
		ur->Undo = [this, id, finishObj, finishAlreadyExisted, oldFinishAssembly]()
		{
			UE_LOG(LogCallTrace, Display, TEXT("ModumateDocument::MakeFinish::Undo"));

			if (finishObj)
			{
				if (finishAlreadyExisted)
				{
					finishObj->ObjectAssembly = oldFinishAssembly;
					finishObj->SetupGeometry();
				}
				else
				{
					DeleteObjectImpl(finishObj);
				}
			}
		};
	};
...
void FModumateDocument::GetObjectIdsByAssembly(const FName &assemblyKey, TArray<int32> &outIds) const
{
	...
	{
		if (moi->ObjectAssembly.DatabaseKey == assemblyKey)
		{
			outIds.Add(moi->ID);
		}
	}
...
bool FModumateDocument::Load(UWorld *world, const FString &path, bool setAsCurrentProject)
{
	...
	if (FModumateSerializationStatics::TryReadModumateDocumentRecord(world,path,docHeader, docRec))
	{
		...
		{
			FModumateObjectInstance *parent = GetObjectById(obj->Parent);
			if (parent != 0)
			{
				obj->Parent = 0;
				parent->AddChild(obj);
...
void FModumateDocument::DisplayDebugInfo(UWorld* world)
{
	...
	Algo::Transform(ObjectInstanceArray, asms,
		[](const FModumateObjectInstance *ob)
		{
			return ob->ObjectAssembly.UniqueKey().ToString();
		}
	);
	...
	for (auto &a : asms)
	{
		int32 instanceCount = Algo::Accumulate(
			...
			{
				return ob->ObjectAssembly.UniqueKey().ToString() == a ? total + 1 : total;
			}
		);
	...
	if (emPlayerState->SelectedObjects.Num() > 0)
	{
		...
		{
			selected += FString::Printf(TEXT("SEL: %d #CHILD: %d PARENTID: %d"), sel->ID, sel->Children.Num(),sel->Parent);
		}
	}
	displayMsg(selected);
}
		...
		else
		{
			...
			if (ensureAlways(pReplacement != nullptr))
			{
				...
				ur->Redo = [this, world, ur, affectedObs, db, replacementAssembly, originalAssembly]()
				{
					...
					{
						ob->ObjectAssembly = replacementAssembly;
						ob->OnAssemblyChanged();
					}
					db->RemoveData(originalAssembly);
					...
					ur->Undo = [this, world, affectedObs, originalAssembly, db]()
					{
						...
						for (auto &ob : affectedObs)
						{
							ob->ObjectAssembly = originalAssembly;
							ob->OnAssemblyChanged();
						}
					};
				};
	...
	const FModumateObjectAssembly &assembly)
{
	...
	for (auto &moi : mois)
	{
		if (moi->ObjectAssembly.DatabaseKey == assembly.DatabaseKey)
		{
			moi->ObjectAssembly = assembly;
