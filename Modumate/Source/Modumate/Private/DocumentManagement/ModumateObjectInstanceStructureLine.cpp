...
namespace Modumate
{
	...
	void FMOIStructureLine::GetDraftingLines(const TSharedPtr<FDraftingComposite>& ParentPage, const FPlane& Plane,
		const FVector& AxisX, const FVector& AxisY, const FVector& Origin, const FBox2D& BoundingBox,
		TArray<TArray<FVector>>& OutPerimeters) const
	{
		static const Units::FThickness lineThickness = Units::FThickness::Points(0.25f);
				...
				for (const auto& edge: draftEdges)
				{
					...
					if (UModumateFunctionLibrary::ClipLine2DToRectangle(vert0, vert1, BoundingBox, boxClipped0, boxClipped1))
					{
						TSharedPtr<FDraftingLine> line = MakeShareable(new FDraftingLine(
							...
							Units::FThickness::Points(0.25f), FMColor::Black));
						ParentPage->Children.Add(line);
						line->SetLayerTypeRecursive(FModumateLayerType::kBeamColumnCut);
					}
			...
			for (const auto& clippedLine : clippedLines)
			{
				...
				if (UModumateFunctionLibrary::ClipLine2DToRectangle(vert0, vert1, BoundingBox, boxClipped0, boxClipped1))
				{
					TSharedPtr<FDraftingLine> line = MakeShareable(new FDraftingLine(
						Units::FCoordinates2D::WorldCentimeters(boxClipped0),
						Units::FCoordinates2D::WorldCentimeters(boxClipped1),
						Units::FThickness::Points(bool(clippedLine.Count) ? 0.15f : 0.05f),
						FMColor::Gray128));
					ParentPage->Children.Add(line);
					line->SetLayerTypeRecursive(FModumateLayerType::kBeamColumnBeyond);
				}
			}
	...
	TArray<FEdge> FMOIStructureLine::GetBeyondLinesFromMesh(const TSharedPtr<FDraftingComposite>& ParentPage, const FPlane& Plane,
