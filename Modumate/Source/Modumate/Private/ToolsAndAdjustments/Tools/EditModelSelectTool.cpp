...
namespace Modumate
{
	...
	void FSelectedObjectToolMixin::AcquireSelectedObjects()
	{
		...
		AEditModelPlayerState_CPP *playerState = SOTMController->EMPlayerState;
		...
		for (auto &kvp : OriginalObjectData)
		{
			...
			kvp.Key->ShowAdjustmentHandles(SOTMController, false);
	...
	FSelectTool::FSelectTool(AEditModelPlayerController_CPP *pc)
	...
	bool FSelectTool::Activate()
	{
		...
		for (auto &ob : sels)
		{
			ob->ShowAdjustmentHandles(Controller.Get(), true);
		}
	...
	bool FSelectTool::HandleMouseUp()
	{
		if (Dragging)
		{
			ProcessDragSelect();
			...
			if (Dragging)
			{
				...
				Algo::Transform(
					...
					[this](const FVector2D &dragCornerScreenPoint) 
					{
						...
						if (UGameplayStatics::DeprojectScreenToWorld(Controller.Get(), dragCornerScreenPoint, dragCornerWorldPos, dragCornerWorldDir))
						{
	...
	bool FSelectTool::HandleSpacebar()
	{
		if (!Active)
		{
			return false;
	...
	bool FSelectTool::ProcessDragSelect()
	{
		FVector2D curMousePosition;
			...
			for (const auto &kvp : snappingView->SnapIndicesByObjectID)
			{
				...
				for (int32 i = cornerStartIndex; i < (cornerStartIndex + numCorners); ++i)
				{
					...
					if (ensureAlways(snapCorner.ObjID == objectID) &&
						UGameplayStatics::ProjectWorldToScreen(Controller.Get(), snapCornerPos, snapCornerScreenPos) &&
				...
				if (!requireEnclosure && !objInSelection)
				{
					...
					for (int32 i = lineStartIndex; i < (lineStartIndex + numLines); ++i)
					{
						...
						if (ensureAlways(snapLine.ObjID == objectID) &&
							UGameplayStatics::ProjectWorldToScreen(Controller.Get(), snapLine.P1, snapLineScreenStart) &&
							UGameplayStatics::ProjectWorldToScreen(Controller.Get(), snapLine.P2, snapLineScreenEnd) &&
	...
	{
		return new FSelectTool(controller);
	}
