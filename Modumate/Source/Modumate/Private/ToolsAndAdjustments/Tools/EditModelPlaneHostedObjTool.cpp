...
namespace Modumate
{
	FPlaneHostedObjTool::FPlaneHostedObjTool(AEditModelPlayerController_CPP *InController,
		EObjectType InObjectType, EAxisConstraint InAxisConstraint)
		: FMetaPlaneTool(InController, InAxisConstraint)
	...
	FPlaneHostedObjTool::~FPlaneHostedObjTool() {}

	bool FPlaneHostedObjTool::ValidatePlaneTarget(const FModumateObjectInstance *PlaneTarget)
	{
		return (PlaneTarget != nullptr) && (PlaneTarget->ObjectType == EObjectType::OTMetaPlane);
	}

	float FPlaneHostedObjTool::GetDefaultJustificationValue()
	{
	...
	bool FPlaneHostedObjTool::Activate()
	{
		if (!FMetaPlaneTool::Activate())
		{
			return false;
		}
	...
	bool FPlaneHostedObjTool::FrameUpdate()
	{
		if (!FMetaPlaneTool::FrameUpdate())
		{
			return false;
		}
	...
	bool FPlaneHostedObjTool::Deactivate()
	{
		...
		return FMetaPlaneTool::Deactivate();
	}

	bool FPlaneHostedObjTool::HandleInputNumber(double n)
	{
		return FMetaPlaneTool::HandleInputNumber(n);
	}

	bool FPlaneHostedObjTool::HandleSpacebar()
	{
		if (!IsInUse())
		{
			return false;
	...
	bool FPlaneHostedObjTool::BeginUse()
	{
		// TODO: require assemblies for stairs, once they can be crafted
		if (!Controller.IsValid() || (Assembly.Key.IsNone() && (ObjectType != EObjectType::OTStaircase)))
		{
			return false;
		}
		...
		if (!PendingObjMesh.IsValid() && !PendingSegment.IsValid() && (LastValidTargetID != MOD_ID_NONE))
		{
			...
			if (!FEditModelToolBase::BeginUse())
			{
				return false;
			}
	...
	bool FPlaneHostedObjTool::EndUse()
	{
		if (PendingObjMesh.IsValid())
		{
			PendingObjMesh->Destroy();
		...
		return FMetaPlaneTool::EndUse();
	}

	bool FPlaneHostedObjTool::AbortUse()
	{
		if (PendingObjMesh.IsValid())
		{
			PendingObjMesh->Destroy();
		...
		return FMetaPlaneTool::AbortUse();
	}

	bool FPlaneHostedObjTool::EnterNextStage()
	{
		return FMetaPlaneTool::EnterNextStage();
	}

	void FPlaneHostedObjTool::SetAssembly(const FShoppingItem &key)
	{
		FMetaPlaneTool::SetAssembly(key);
	...
	bool FPlaneHostedObjTool::MakeObject(const FVector &Location, TArray<int32> &newObjIDs)
	{
		...
		bool bSuccess = FMetaPlaneTool::MakeObject(Location, newPlaneIDs);
	...
	{
		return new FPlaneHostedObjTool(controller, EObjectType::OTWallSegment, EAxisConstraint::AxisZ);
	}

	IModumateEditorTool *MakeFloorTool(AEditModelPlayerController_CPP *controller)
	{
		return new FPlaneHostedObjTool(controller, EObjectType::OTFloorSegment, EAxisConstraint::AxesXY);
	}
}
