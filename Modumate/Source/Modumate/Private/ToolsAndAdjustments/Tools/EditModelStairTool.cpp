...
namespace Modumate
{
	FStairTool::FStairTool(AEditModelPlayerController_CPP *InController)
	...
	FStairTool::~FStairTool() {}

	bool FStairTool::Activate()
	{
	...
	bool FStairTool::HandleInputNumber(double n)
	{
		return false;
	}

	bool FStairTool::BeginUse()
	{
	...
	bool FStairTool::EnterNextStage()
	{
		FSnappedCursor &cursor = Controller->EMPlayerState->SnappedCursor;
		if (!cursor.Visible)
		{
	...
	void FStairTool::SetAssembly(const FShoppingItem &key)
	{
	...
	bool FStairTool::UpdatePreviewStairs()
	{
	...
	bool FStairTool::MakeStairs(int32 &RefParentPlaneID, int32 &OutStairsID)
	{
	...
	bool FStairTool::ValidatePlaneTarget(const FModumateObjectInstance *PlaneTarget)
	{
	...
	void FStairTool::MakePendingSegment(TWeakObjectPtr<ALineActor3D_CPP> &TargetSegment, const FVector &StartingPoint, const FColor &SegmentColor)
	{
	...
	{
		return new FStairTool(controller);
	}
}
