...
namespace Modumate
{
	FJoinTool::FJoinTool(AEditModelPlayerController_CPP *InController)
	...
	bool FJoinTool::BeginUse()
	{
		return FEditModelToolBase::BeginUse();
	}

	bool FJoinTool::HandleMouseUp()
	{
		...
		if (newTarget && newTarget->ObjectType == EObjectType::OTMetaPlane)
		{
			...
			newTarget->ShowAdjustmentHandles(Controller.Get(), false);
		}
	...
	{
		return new FJoinTool(controller);
	}
}