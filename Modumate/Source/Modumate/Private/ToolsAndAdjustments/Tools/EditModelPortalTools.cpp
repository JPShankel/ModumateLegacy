...
#include "EditModelPortalTools.h"
...
namespace Modumate
{
	FPortalToolBase::FPortalToolBase(AEditModelPlayerController_CPP *pc)
		: Controller(pc)
		, CursorActor(nullptr)
		, Document(nullptr)
		, HostID(0)
	...
	{
		Document = &Controller->GetWorld()->GetGameState<AEditModelGameState_CPP>()->Document;
	}

	FPortalToolBase::~FPortalToolBase() {}

	bool FPortalToolBase::Activate()
	{
		HostID = 0;
	...
	void FPortalToolBase::SetAssembly(const FShoppingItem &key)
	{
		Assembly = key;
		if (Active)
		{
	...
	void FPortalToolBase::SetupCursor()
	{
		...
		if (obAsm && obAsm->PortalConfiguration.IsValid())
		{
			...
			if (Controller->EMPlayerState->ToolToMode.Contains(this))
			{
				CursorActor->TempAssemblyKey = Assembly.Key;
				CursorActor->TempObjectToolMode = Controller->EMPlayerState->ToolToMode[this];
			}
		}
	}

	bool FPortalToolBase::Deactivate()
	{
		if (CursorActor != nullptr)
		{
			CursorActor->Destroy();
	...
	bool FPortalToolBase::FrameUpdate()
	{
		...
		if (Controller->EMPlayerState->GetToolMode() == EToolMode::VE_DOOR)
		{
			heightFromBottom = Modumate::Units::FUnitValue::WorldInches(Document->GetDefaultDoorHeight()).AsWorldInches();
		}
		else if (Controller->EMPlayerState->GetToolMode() == EToolMode::VE_WINDOW)
		{
	...
	bool FPortalToolBase::EnterNextStage()
	{
		return true;
	}

	bool FPortalToolBase::EndUse()
	{
		return true;
	}

	bool FPortalToolBase::ScrollToolOption(int32 dir)
	{
		return true;
	}

	bool FPortalToolBase::HandleInputNumber(double n)
	{
		return true;
	}

	bool FPortalToolBase::AbortUse()
	{
		return true;
	}

	bool FPortalToolBase::HandleSpacebar()
	{
		if (!IsInUse())
		{
			return false;
	...
	bool FPortalToolBase::HandleControlKey(bool pressed)
	{
		return true;
	}
	...
	FDoorTool::FDoorTool(AEditModelPlayerController_CPP *pc) : FPortalToolBase(pc)
	{
	};

	bool FDoorTool::BeginUse()
	{
	...
	FWindowTool::~FWindowTool() {};

	FWindowTool::FWindowTool(AEditModelPlayerController_CPP *pc)
		: FPortalToolBase(pc)
	{
	}

	bool FWindowTool::BeginUse()
	{
		if (HostID != 0 && !bHasBoringError)
		{
			Controller->ModumateCommand(
