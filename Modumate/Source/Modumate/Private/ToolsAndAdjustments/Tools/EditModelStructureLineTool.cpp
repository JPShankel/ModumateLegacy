...
namespace Modumate
{
	FStructureLineTool::FStructureLineTool(AEditModelPlayerController_CPP *InController)
	...
	FStructureLineTool::~FStructureLineTool() {}

	bool FStructureLineTool::Activate()
	{
	...
	bool FStructureLineTool::HandleInputNumber(double n)
	{
		return false;
	}

	bool FStructureLineTool::Deactivate()
	{
		ResetState();
	...
	bool FStructureLineTool::BeginUse()
	{
		if (!Assembly.Valid)
		{
			return false;
	...
	bool FStructureLineTool::EnterNextStage()
	{
	...
	bool FStructureLineTool::FrameUpdate()
	{
	...
	bool FStructureLineTool::EndUse()
	{
		ResetState();
		return FEditModelToolBase::EndUse();
	}

	bool FStructureLineTool::AbortUse()
	{
		ResetState();
		return FEditModelToolBase::AbortUse();
	}

	void FStructureLineTool::SetAssembly(const FShoppingItem &key)
	{
		FEditModelToolBase::SetAssembly(key);
	...
	void FStructureLineTool::SetCreateObjectMode(EToolCreateObjectMode InCreateObjectMode)
	{
		FEditModelToolBase::SetCreateObjectMode(InCreateObjectMode);
	...
	void FStructureLineTool::SetTargetID(int32 NewTargetID)
	{
	...
	bool FStructureLineTool::SetStructureLineHidden(int32 StructureLineID, bool bHidden)
	{
	...
	bool FStructureLineTool::UpdatePreviewStructureLine()
	{
	...
	bool FStructureLineTool::MakeStructureLine(int32 TargetEdgeID)
	{
	...
	void FStructureLineTool::ResetState()
	{
		if (PendingSegment.IsValid())
		{
	...
	{
		return new FStructureLineTool(controller);
	}
}
