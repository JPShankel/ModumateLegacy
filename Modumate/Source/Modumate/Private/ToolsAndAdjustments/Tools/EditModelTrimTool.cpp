...
namespace Modumate
{
	FTrimTool::FTrimTool(AEditModelPlayerController_CPP *controller)
	...
	bool FTrimTool::HasValidTarget() const
	{
	...
	bool FTrimTool::ConstrainTargetToSolidEdge(float targetPosAlongEdge,int32 startIndex,int32 endIndex)
	{
	...
	bool FTrimTool::HandleInputNumber(double n)
	{
		return false;
	}

	bool FTrimTool::Activate()
	{
	...
	bool FTrimTool::AbortUse()
	{
		EndUse();
		return FEditModelToolBase::AbortUse();
	}

	bool FTrimTool::HandleSpacebar()
	{
		if (!IsActive())
		{
			return false;
	...
	void FTrimTool::SetAssembly(const FShoppingItem &key)
	{
	...
	{
		return new FTrimTool(controller);
	}
}
