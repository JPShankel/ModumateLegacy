...
namespace Modumate
{
	...
	bool FGraph3DEdge::IntersectsFace(const FGraph3DFace *OtherFace, FVector &IntersectingEdgeOrigin, FVector &IntersectingEdgeDir, TArray<FEdgeIntersection> &DestIntersections, bool &bOutOnFaceEdge) const
	{
		auto startVertex = Graph->FindVertex(StartVertexID);
		auto endVertex = Graph->FindVertex(EndVertexID);
		...
		FVector startPos = startVertex->Position;
		FVector endPos = endVertex->Position;

		if (OtherFace->CachedPlane.PlaneDot(startPos) > Graph->Epsilon)
		{
			return false;
		}
		if (OtherFace->CachedPlane.PlaneDot(endPos) > Graph->Epsilon)
		{
			return false;
		}

		IntersectingEdgeOrigin = startPos;
		IntersectingEdgeDir = (endPos - startPos).GetSafeNormal();
		...
		if (DestIntersections.Num() != 2)
		{
			return false;
		}
		for (auto &intersection : DestIntersections)
		{
			if (FMath::Abs(intersection.DistAlongRay) > Graph->Epsilon &&
				FMath::Abs(intersection.DistAlongRay - (endPos - startPos).Size()) > Graph->Epsilon)
			{
				return false;
			}
		}

		auto vertexA = Graph->FindVertex(DestIntersections[0].Point);
		auto vertexB = Graph->FindVertex(DestIntersections[1].Point);
		...
		if (vertexA != nullptr && vertexB != nullptr)
		{
			...
			if (edge != nullptr)
			{
				...
				bOutOnFaceEdge = bOtherFaceContainsEdge;
			}

			// if faces don't contain edge, make sure that the edge is inside the face
			auto intersection = DestIntersections;
			if (!bOutOnFaceEdge)
			{
				...
				for (int32 idx = 0; idx < 2; idx++)
				{
					...
					FVector edgeNormal = OtherFace->CachedEdgeNormals[edgeIdx];
