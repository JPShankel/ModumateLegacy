...
void AEditModelDatasmithRuntimeActor::OnImportEnd()
{
	...
	for (const auto curStaticMeshComp : staticMeshComps)
	{
		...
		if (mesh)
		{
			...
			StaticMeshMaterialsMap.Add(mesh, curStaticMeshComp->GetMaterials());
			StaticMeshMaterials.Append(curStaticMeshComp->GetMaterials());
		}
	}
...
bool AEditModelDatasmithRuntimeActor::GetValuesFromMetaData()
{
	...
	for (const auto meshMatPair : StaticMeshMaterialsMap)
	{
		...
		for (const auto staticMeshMaterial : meshMatPair.Value)
		{
			...
			// Get original Datasmith Material, use PbrOpaque if null
			UMaterialInterface* originalStaticMeshMat = staticMeshMaterial;
			if (!originalStaticMeshMat)
			{
				originalStaticMeshMat = UMaterialInstanceDynamic::Create(DatasmithImporter->Original_DS_PbrOpaque, this);;
			}
			// Check for material type, and if it is overridden by MetaData
			EDatasmithMaterialType matType = DatasmithImporter->GetTypeFromOriginalDatasmithMaterial(originalStaticMeshMat);
			...
			dynMat->CopyMaterialUniformParameters(originalStaticMeshMat);
			dynMats.Add(dynMat);
			...
			if (dynMat && bUseMetaData)
			{
				...
				{
					UTexture2D* diffuseMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.DiffuseMap);
				...
				{
					UTexture2D* metallicMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.MetallicMap);
				...
				{
					UTexture2D* specularMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.SpecularMap);
				...
				{
					UTexture2D* roughnessMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.RoughnessMap);
				...
				{
					UTexture2D* normalMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.NormalMap);
					if (normalMapTexture)
					{
				...
				{
					UTexture2D* bumpMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.BumpMap);
					if (bumpMapTexture)
					{
				...
				{
					UTexture2D* opacityMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.OpacityMap);
				...
				{
					UTexture2D* transparencyMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.TransparencyMap);
				...
				{
					UTexture2D* cutoutOpacityMapTexture = FImageUtils::ImportFileAsTexture2D(assetFolderPathClean + curMaterialMetaData.CutoutOpacityMap);
