...
void ACompoundMeshActor::MakeFromAssembly(const FBIMAssemblySpec &obAsm, const FVector &scale, bool bLateralInvert, bool bMakeCollision)
{
	...
	{
		float portalXScale = bLateralInvert ? -1.0f : 1.0f;
		float portalMinX = portalConfig.CachedDimensions.FindRef(FPortalConfiguration::RefPlaneNameMinX).AsWorldCentimeters();
		float portalMaxX = portalConfig.CachedDimensions.FindRef(FPortalConfiguration::RefPlaneNameMaxX).AsWorldCentimeters();
		float portalWidth = portalMaxX - portalMinX;

		int32 numSlots = portalConfig.Slots.Num();
		...
		for (int32 slotIdx = 0; slotIdx < numSlots; ++slotIdx)
		{
			...
			if (obAsm.CachedAssembly.PortalParts.Contains(slotIdx))
			{
				...
				UStaticMesh *partMesh = portalPart.Mesh.EngineMesh.Get();
				...
				FVector partNativeSize(ForceInitToZero), partScale(1.0f);
				Units::FUnitValue partLocX, partLocY, partLocZ, partSizeX, partSizeY, partSizeZ;
				if (ensureAlwaysMsgf(
					Expression::Evaluate(portalConfig.CachedDimensions, slot.LocationX, partLocX) &&
					Expression::Evaluate(portalConfig.CachedDimensions, slot.LocationY, partLocY) &&
					Expression::Evaluate(portalConfig.CachedDimensions, slot.LocationZ, partLocZ) &&
					Expression::Evaluate(portalConfig.CachedDimensions, slot.SizeX, partSizeX) &&
					Expression::Evaluate(portalConfig.CachedDimensions, slot.SizeY, partSizeY) &&
					Expression::Evaluate(portalConfig.CachedDimensions, slot.SizeZ, partSizeZ),
					TEXT("Failed to evaluate portal location/size expression(s)!")))
				{
					partLocation.Set(
						partLocX.AsWorldCentimeters(),
						partLocY.AsWorldCentimeters(),
						partLocZ.AsWorldCentimeters()
					);
					partDesiredSize.Set(
						partSizeX.AsWorldCentimeters(),
						partSizeY.AsWorldCentimeters(),
						partSizeZ.AsWorldCentimeters()
					);
					partNativeSize.Set(
						portalPart.NativeSizeX.AsWorldCentimeters(),
						portalPart.NativeSizeY.AsWorldCentimeters(),
						portalPart.NativeSizeZ.AsWorldCentimeters()
					);

					partScale = partDesiredSize / partNativeSize;
				}
				else
				{
				...
				// Skip parts that would be 0-size or less.
				if (partDesiredSize.GetMin() <= 0.0f)
				{
					continue;
				}
				...
				if (bLateralInvert)
				{
					partLocation.X = portalMinX + (portalWidth - (partLocation.X - portalMinX));
				}

				FVector partFlip(
					slot.FlipX ? -portalXScale : portalXScale,
					slot.FlipY ? -1.0f : 1.0f,
					slot.FlipZ ? -1.0f : 1.0f
				);
				FRotator partRotator(slot.RotateY, slot.RotateZ, slot.RotateX);

				FBox nineSliceInterior = portalPart.NineSliceInterior;
